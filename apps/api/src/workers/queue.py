...
async def enqueue_agent_task(command: AgentTaskCommand, max_retries: int = 3) -> Dict[str, Any]:
    """
    ...
    if pool is not None:
        try:
            ...
            result = await job.result(timeout=300, poll_delay=0.5)
            return result
        except Exception as exc:
    ...
    for attempt in range(max_retries):
        try:
            ...
            return await process_agent_task(ctx=None, command_data=command.to_dict())
        except Exception as exc:
            last_error = exc
